...
class FavorSystem:
    ...
    async def gift_item(self, user_id: str, item_name: str) -> dict:
        """送礼物给莫宁宁（需要消耗星声）

        Returns:
            dict: {'success': bool, 'message': str, 'favor_change': int}
        """
        items = self.get_favor_items()

        if item_name not in items:
            return {
                'success': False,
                'message': f'❌ 未知的礼物：{item_name}\n可用礼物：{", ".join(items.keys())}',
                'favor_change': 0
            }
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if balance < price:
                return {
                    'success': False,
                    'message': f'❌ 星声不足！需要 {price} 星声，当前 {balance} 星声',
                    'favor_change': 0
                }
        ...
        if favor_change > 0:
            message = f"✅ 你送给莫宁宁一份【{item_name}】（消耗 {price} 星声）\n💕 好感值 +{favor_change}\n📊 当前好感值：{favor_info['favor_value']}"
        elif favor_change < 0:
            message = f"😅 你送给莫宁宁一份【{item_name}】（消耗 {price} 星声）\n💔 好感值 {favor_change}\n📊 当前好感值：{favor_info['favor_value']}"
        else:
            message = f"😐 你送给莫宁宁一份【{item_name}】（消耗 {price} 星声）\n😐 莫宁宁面无表情\n📊 当前好感值：{favor_info['favor_value']}"
